...
from keras.layers import Concatenate,Activation,Embedding, Flatten, Dense, LSTM, Multiply, Input, Dot, Dropout, Bidirectional,\
 Lambda, Subtract
...
def main(args):
    ...
    for sentence in total_dataset:
        total_p = sentence.get("premise")
        total_c = sentence.get("claim")
        total_w = sentence.get("warrant")
        labels = sentence.get("label")
