...
#        total_w.append(sentence["warrant"])
    ...
    for claim, premise, label in zip(padded_claim, padded_premise, labels):
        ...
        score_1 = [row[1] for row in score]
        #new_score = np.reciprocal(score_1)
        label_score = [(i,j) for i, j in zip(label, score_1)]
        sorted_score = sorted(label_score, key=lambda tup:(-tup[1], tup[0]))
        index_1 = [i for i, tupl in enumerate(sorted_score) if tupl[0] == 1]
        reciprocal_rank = (1/(index_1[0] + 1))
